    ...
    <div v-for="profile in profiles" :key="profile.id" class="editable-field">
      <template v-if="editedFieldId === profile.id">
        ...
        <form>
          ...
          <div class="form-group m-3">
             <input type="text" v-model="profile.dob" :ref="`field${profile.id}`" class="form-control" />
          </div>
          <div class="form-group m-3">
...
export default {
   data(){
         return{
                editedFieldId: null,
               profiles:[{id:1, username:"esther001",email:'example@email.com', city:"London",dob:"06/12/2000",bio:"HI my name is x and I am from London, I study at QMUL and my hobbies are..."},
